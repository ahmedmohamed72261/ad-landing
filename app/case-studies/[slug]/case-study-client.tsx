...
export default function CaseStudyClient({ params }: Props) {
  ...
  const formattedResults = caseStudy.results.map(
    (result) => `${result.metric}: تحسن بنسبة ${result.value} - ${result.improvement}`,
  )
  ...
  const sections = [
    ...
    },
    {
      ...
      bullets: formattedResults, // Use formatted strings instead of objects
      icon: CheckCircle2,
    },
    {
      title: "الخلاصة",
